...
#                ,[119, 10, 32]]
                # 18 = bicycle
label_colours = [tuple([0,0,0]), tuple([255,0,0]),tuple([0,255,0]),tuple([0,0,255]),tuple([255,255,0]),tuple([255,0,255]),tuple([0,255,255]),tuple([255,255,255])]
...
def decode_labels(mask, img_shape, num_classes, style_color):
    '''
        style color: "111111110"(color in range) + "n"(max color)
    '''
    ...
    maxColor = int(style_color[-1])

    style_color = style_color[:-1]
    tmpLabel_colours = label_colours[:]
    for i in np.arange(8):
        if style_color[7-i] == "0":
            del tmpLabel_colours[7-i]

    for i in np.arange(150):
        
        if color_table[i] not in tmpLabel_colours:
            color_table[i] = label_colours[maxColor]
